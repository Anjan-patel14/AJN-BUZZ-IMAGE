...
export function ToolCatalog({ mode = 'all' }: { mode?: 'all' | 'favorites' | 'recent' }) {
  ...
  const tools = useMemo(() => {
    ...
    return IMAGE_TOOLS.filter(tool => {
      ...
      return `${tool.name} ${tool.summary} ${tool.description} ${tool.formats} ${tool.seoKeywords.join(' ')}`.toLowerCase().includes(normalized);
  ...
  return <>
    <div className="catalog-surface ajn-glass-card">
      <div className="catalog-heading">
        <div><div className="eyebrow">Image tools</div><h2>Choose a tool</h2></div>
        <span className="catalog-accent" aria-hidden="true"/>
      </div>
      <div className="catalog-toolbar">
        <label className="searchbox">
          <Search size={17}/>
          <input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search tools" aria-label="Search image tools" />
        </label>
    ...
    {tools.length ? <div className="tool-grid">
      {tools.map(tool => {
        ...
        return <article className={`tool-card ajn-tool-card-pro ${t.card}`} key={tool.id}>
          <button
          ...
          <Link href={`/tools/${tool.id}`} className="tool-card-link" prefetch={false}>
            <div className="tool-card-top">
              <div className={`tool-icon ${t.icon}`}><ToolIcon name={tool.icon}/></div>

...
Write-Host ' AJN BUZZ IMAGE V5.0 :: SEO + ADS + IMAGE PRODUCTION PREVIEW' -ForegroundColor Cyan
Write-Host ' AJN PDF UI + REAL TARGET COMPRESSION + LIVE SITEMAP' -ForegroundColor Cyan
Write-Host ' NO WORKSPACE | NO LOGIN | NO BILLING | NO DEPLOY' -ForegroundColor Yellow
Write-Host '============================================================' -ForegroundColor Cyan
...
if(!(Test-Path '.env.local')){Copy-Item '.env.example' '.env.local';Write-Host '[INFO] Created .env.local from production-safe example values.' -ForegroundColor Yellow}
...
if($pkg.version -ne '5.0.0'){throw "Expected AJN Buzz 5.0.0, found $($pkg.version)"}
...
if(!(Test-Path $tsc)){
  ...
  Write-Host '[INFO] Reusing existing node_modules + TypeScript toolchain.' -ForegroundColor Green
}
...
Write-Host '[INFO] Running source verification + TypeScript + optimized production build...' -ForegroundColor Cyan
npm.cmd run check
if($LASTEXITCODE -ne 0){throw 'AJN Buzz full production check failed'}
Write-Host '[PASS] Source + TypeScript + production build checks complete' -ForegroundColor Green
...
$ready=$false
for($i=0;$i -lt 120;$i++){try{$r=Invoke-WebRequest 'http://localhost:9010/api/health' -UseBasicParsing -TimeoutSec 3;if($r.StatusCode -eq 200){$ready=$true;break}}catch{};Start-Sleep 1}
...
Write-Host ''
Write-Host '[PASS] AJN BUZZ IMAGE V5.0 PRODUCTION PREVIEW READY' -ForegroundColor Green
